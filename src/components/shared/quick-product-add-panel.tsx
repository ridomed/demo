...
export function QuickProductAddPanel<T extends QuickProduct>({
  products,
  onAddProducts,
}: {
  products: T[];
  onAddProducts: (products: T[]) => void;
}) {
  ...
  const categories: Option[] = [
    { id: "", name: t.invoices.selectCategoryPlaceholder },
    ...Array.from(
      new Map(
        products.map((p) => [
          p.categoryId,
  ...
  const brands: Option[] = [
    { id: "", name: t.invoices.selectBrandPlaceholder },
    ...Array.from(
      new Map(
        products
          .filter((p) => p.brandId)
      ...
      {(categoryId || brandId) && (
        <>
          ...
          <Button
            ...
            onClick={() => {
              onAddProducts(filtered.filter((p) => selected.has(p.id)));
              setSelected(new Set());
            }}
          >
            <Plus className="size-4" />
            {formatMessage(t.invoices.addSelectedToInvoice, {
              count: selected.size.toLocaleString(locale),
            })}
          </Button>
        </>
      )}
